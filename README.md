...
- **Accessor properties are storage-only.** `Property(T)` carries typed `getter`/`setter` slots (`?T`, so they can hold real function values — e.g. z-value's `JSValue` closures), populated via `defineAccessor(key, getter, setter, empty_value)` (get+set for the same key merge into one record) and inspected via `getOwnRecord`/`isAccessor()`. But *invoking* an accessor requires an interpreter this repo doesn't know about — `get()`/`values()`/`entries()` and every other data-path API see the caller-supplied `empty_value` instead. [z-interpreter](https://github.com/carlos-sweb/z-interpreter) does the actual dispatch. (The old context-free fn pointers on `PropertyDescriptor` remain a dead v1 sketch, untouched.)
